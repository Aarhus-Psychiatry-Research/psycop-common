...
df_first_administered_diabetes_medication <- con %>% 
  ...
  filter(datotid_ordination_start > event_start_date_str) %>% 
  group_by(dw_ek_borger) %>% 
  filter(datotid_ordination_start == min(datotid_ordination_start)) %>% 
  collect %>% 
  format_sql_import() %>% 
  group_by(dw_ek_borger) %>% 
  distinct(dw_ek_borger, datotid_ordination_start) %>%  # Drop if two diabetes medications were started on the same day
  ungroup()
```
...
df_first_prescribed_diabetes_medication <- con %>% 
  ...
  filter(datotid_ordinationstart > event_start_date_str) %>% 
