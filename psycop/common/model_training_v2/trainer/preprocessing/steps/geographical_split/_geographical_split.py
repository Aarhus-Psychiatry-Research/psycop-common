...
def load_shak_to_location_mapping() -> pl.DataFrame:
    ...
    return (
        pl.read_csv(Path(__file__).parent / "shak_mapping.csv")
        .with_columns(pl.col("shak_6").cast(str))
...
def add_migration_date_by_patient(
    first_visit_at_first_region_df: pl.DataFrame, first_visit_at_second_region_df: pl.DataFrame
) -> pl.DataFrame:
    ...
    return (
        first_visit_at_first_region_df.join(
            first_visit_at_second_region_df, how="left", on="dw_ek_borger"
        ).drop("timestamp")
    ).with_columns(
...
def add_shak_to_region_mapping(
    visits: pl.DataFrame, shak_to_location_df: pl.DataFrame, shak_codes_to_drop: Collection[str]
) -> pl.DataFrame:
