...
class OptunaHyperParameterOptimization:
    ...
    @staticmethod
    def _check_if_suggester_in_configspace(cfg: dict[str, Any]):
        ...
        for _, value in cfg.items():
            if isinstance(value, (SuggesterSpace, Suggester)):
                ...
                if OptunaHyperParameterOptimization()._check_if_suggester_in_configspace(
                    value
                ):
                    return True
        return False
    ...
    def _resolve_only_suggestors(cfg: dict[str, Any]) -> dict[str, Any]:
        """Resolves only suggestors in a nested config. Suggestors are identified
        by the presence of the @suggestors key. They need to be resolved at the level
        where they are defined, i.e. where @suggesters is a value in the dict."""
    ...
    def _optuna_objective(
        trial: Trial, cfg_with_resolved_suggesters: dict[str, Any]
    ) -> float:
