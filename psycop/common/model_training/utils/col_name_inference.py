...
def get_col_names(
    cfg: Union[DictConfig, FullConfigSchema], dataset: pd.DataFrame
    ...
    potential_outcome_col_names = [
        ...
        if cfg.data.outc_prefix in c and str(cfg.preprocessing.pre_split.min_lookahead_days) in c
    ]
    ...
    train_col_names = [
        c for c in dataset.columns if c.startswith(cfg.data.pred_prefix) and "uuid" not in c
    ]
...
def infer_look_distance(
    ...
    if not isinstance(col_name, str):
        for c_name in col_name:
            look_distances += infer_look_distance(col_name=c_name, regex_pattern=regex_pattern)
    else:
        look_distances = re.findall(pattern=regex_pattern, string=col_name)

    if len(look_distances) > 1 and not allow_multiple:
        raise ValueError(f"Multiple col names provided and allow_multiple is {allow_multiple}.")

    return look_distances


def infer_col_names(df: pd.DataFrame, prefix: str, allow_multiple: bool = True) -> list[str]:
    ...
    if len(col_name) > 1:
        if allow_multiple:
            return col_name
        raise ValueError(f"Multiple columns found and allow_multiple is {allow_multiple}.")
    if not col_name:
...
def infer_predictor_col_name(
    df: pd.DataFrame, prefix: str = "pred_", allow_multiple: bool = True
) -> list[str]:
...
def infer_y_hat_prob_col_name(
    df: pd.DataFrame, prefix: str = "y_hat_prob", allow_multiple: bool = False
) -> list[str]:
