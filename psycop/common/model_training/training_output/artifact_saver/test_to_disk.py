...
def test_eval_dataset_to_disk_series(tmp_path: Path):
    ...
    eval_ds=EvalDataset(ids=ids,
                        pred_time_uuids=pred_time_uuids,
                        pred_timestamps=pred_timestamps,
                        y=y,
                        y_hat_probs=y_hat_probs)

    ArtifactsToDiskSaver.eval_dataset_to_disk(eval_dataset=eval_ds, file_path=tmp_path.with_suffix(".parquet"))
...
def test_eval_dataset_to_disk_df(tmp_path: Path):
    ...
    y_hat_probs=pd.DataFrame({"y_hat_probs_1": [0, 0.2, 0.5, 0], "y_hat_probs_2": [0.5, 0, 0, 0.9]})
