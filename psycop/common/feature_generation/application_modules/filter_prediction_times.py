...
class PredictionTimeFilterer:
    ...
    def __init__(
        ...
        if len(uuid_cols) == 0:
            ...
            self.prediction_times_df[self.pred_time_uuid_col_name] = self.prediction_times_df[
                self.entity_id_col_name
            ].astype(str) + self.prediction_times_df[timestamp_col_name].dt.strftime(
                "-%Y-%m-%d-%H-%M-%S"
            )
    ...
    def _filter_prediction_times_by_quarantine_period(self) -> pd.DataFrame:
        ...
        if self.quarantine_df is None:
            raise ValueError("quarantine_df must be provided to filter by quarantine period.")

        df = self.prediction_times_df.merge(
            self.quarantine_df, on=self.entity_id_col_name, how="left"
        )
        ...
        df = df.drop(columns=["hit_by_quarantine"])

        n_after = len(df)
        log.info(f"Filtered {n_before - n_after} prediction times by quarantine period.")
