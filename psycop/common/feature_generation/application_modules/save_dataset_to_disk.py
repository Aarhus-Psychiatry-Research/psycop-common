...
def save_chunk_to_disk(project_info: ProjectInfo, flattened_df_chunk: pd.DataFrame, chunk: int):
    """Save split to disk."""
...
def save_split_to_disk(split_df: pd.DataFrame, split_name: str, feature_set_dir: Path):
    """Save split to disk."""
    # Version table with current date and time
...
def filter_by_split_ids(
    ...
    ids_in_split_but_not_in_flattened_df = split_ids[~np.isin(split_ids, flattened_df_ids)]
...
def split_and_save_dataset_to_disk(
    ...
    for split_name in split_names:
        ...
        save_split_to_disk(
            split_df=split_df, split_name=split_name.value, feature_set_dir=feature_set_dir
        )
