...
def auroc_by_outcome(model_names: list[str], validation_outcomes: list[pl.DataFrame], y_hat_col_name: str = "y_hat_prob", best_run_metric: str = "all_oof_BinaryAUROC") -> pd.DataFrame:
    ...
    for model_name, validation_outcome_col_name in models_by_outcomes:
        ...
        performance_dfs.append(pd.DataFrame({"model_name": model_name, "validation_outcome": validation_outcome_col_name, "estimate": [round(roc_auc_score( # type: ignore
...
def plot_auroc_by_outcome(df: pd.DataFrame, x_axis_label: str = "Validation outcome", y_axis_label: str = "Training outcome", plot_title: str = "AUROC by Outcome") -> pn.ggplot:
    p = (
