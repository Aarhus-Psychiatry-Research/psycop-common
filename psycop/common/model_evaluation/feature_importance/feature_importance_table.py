...
def generate_feature_importances_table(
    feature_importance_dict: dict[str, float], output_format: str = "df"
) -> Union[pd.DataFrame, str]:
    """Generate table with feature importances.
    Args:
        feature_importance_dict (dict[str, float]): Dictionary with feature importances
        output_format (str, optional): The output format. Takes one of "html", "df". Defaults to "df".
    Returns:
        Union[pd.DataFrame, str]: The table with feature importances
    """
...
def generate_selected_features_table(
    selected_features_dict: dict[str, bool], output_format: str = "df", removed_first: bool = True
) -> Union[pd.DataFrame, str]:
