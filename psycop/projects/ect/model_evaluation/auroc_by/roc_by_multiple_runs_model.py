...
from psycop.projects.ect.model_evaluation.single_run_artifact import RunSelector
...
def _run_auroc_df(df: pl.DataFrame) -> pl.DataFrame:
    ...
    auroc_df = pl.DataFrame({"fpr" : fpr, "tpr" : tpr}).with_columns(pl.lit(df["run_name"][0]).alias("run_name"), pl.lit(roc_auc_score(y_true=df["y"], y_score=df["y_hat_prob"])).alias("AUROC"))
    return auroc_df
