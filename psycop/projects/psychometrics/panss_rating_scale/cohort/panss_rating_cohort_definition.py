...
from psycop.projects.psychometrics.panss_rating_scale.cohort.eligible_prediction_times.single_filters import (
    PanssRatingF2disorders,
    PanssRatingWashoutMoveFilter,
...
@shared_cache().cache()
def panss_rating_pred_filtering() -> FilteredPredictionTimeBundle:
    return PanssRatingCohortDefiner().get_filtered_prediction_times_bundle()


@shared_cache().cache()
def panss_rating_pred_times() -> PredictionTimeFrame:
    return panss_rating_pred_filtering().prediction_times


@shared_cache().cache()
def panss_rating_outcome_timestamps() -> OutcomeTimestampFrame:
    return PanssRatingCohortDefiner().get_outcome_timestamps()


class PanssRatingCohortDefiner(CohortDefiner):
    @staticmethod
...
if __name__ == "__main__":
    filtered_prediction_time_bundle = (
        PanssRatingCohortDefiner.get_filtered_prediction_times_bundle()
    )

    outcome_timestamps_bundle = PanssRatingCohortDefiner.get_outcome_timestamps()
