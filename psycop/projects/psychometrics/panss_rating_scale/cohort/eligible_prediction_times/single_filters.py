...
from psycop.projects.psychometrics.panss_rating_scale.cohort.eligible_prediction_times.add_age import (
    add_age,
)
from psycop.projects.psychometrics.panss_rating_scale.cohort.eligible_prediction_times.eligible_config import (
    AGE_COL_NAME,
    MIN_AGE,
    MIN_DATE,
)
from psycop.projects.psychometrics.panss_rating_scale.cohort.eligible_prediction_times.f2_disorders import (
    add_only_patients_with_f2_diagnosis,
)
...
class PanssRatingF2disorders(PredictionTimeFilter):
    def apply(self, df: pl.LazyFrame) -> pl.LazyFrame:
        f2_disorders_df = (
            add_only_patients_with_f2_diagnosis()
            .lazy()
            .select(pl.col("timestamp").alias("timestamp_f2"), pl.col("dw_ek_borger"))
        )

        prediction_times_with_f2_disorders_df = df.join(
            f2_disorders_df, on="dw_ek_borger", how="inner"
        )

        days_in_1_year = 1 * 365

        valid_timestamp_f2_disorder = prediction_times_with_f2_disorders_df.filter(
            (pl.col("timestamp_f2") <= pl.col("timestamp"))
            & (pl.col("timestamp_f2") >= (pl.col("timestamp") - pl.duration(days=days_in_1_year)))
        ...
        after_df = (
            valid_timestamp_f2_disorder.groupby("dw_ek_borger")
...
class PanssRatingWashoutMoveFilter(PredictionTimeFilter):
    def apply(self, df: pl.LazyFrame) -> pl.LazyFrame:
