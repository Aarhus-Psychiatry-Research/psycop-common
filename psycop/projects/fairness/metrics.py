...
def bootstrap_metrics(
    ...
    if len(eval_df[protected_attribute].unique()) < 3:
        ...
        ratio_missing = (
            ratio_results.groupby(["variable"])["value"]
            .apply(lambda x: x.isna().mean())
            .reset_index()
            .rename(columns={"value": "missing"})
        )
        ...
        ratio_estimates = pd.concat(
            [estimate_metrics.ratio().reset_index(), estimate_auroc.ratio().reset_index()]
        ...
        ratio_missing = (
            ratio_results.groupby([protected_attribute, "variable"])["ratio"]
            .apply(lambda x: x.isna().mean())
            .reset_index()
            .rename(columns={"ratio": "missing"})
        )
        ...
        estimates_overall = pd.concat(
            [estimate_metrics.overall.reset_index(), estimate_auroc.overall.reset_index()]
        ).rename(columns={"index": "variable", 0: "overall"})
        ...
        ratio_estimates = ratio_estimates.drop(columns=["value", "overall"]).rename(
            columns={"ratio": "value"}
        )

        ratio_estimates = pd.merge(
            ratio_results, ratio_estimates, on=[protected_attribute, "variable"]
        )

        ratio_estimates = pd.merge(
            ratio_estimates, ratio_missing, on=[protected_attribute, "variable"]
        )
...
if __name__ == "__main__":
    ...
    for model in eval_df.model.unique():
        by_group, ratio = bootstrap_metrics(
            eval_df[eval_df["model"] == model],
            metrics=metrics,
    ...
    by_group_boot["cohort_n"] = by_group_boot.groupby(["model", "variable"])["Count"].transform(
        "sum"
    )
    ...
    ect_boots = bootstrap_metrics(
        eval_df[eval_df["model"] == "ECT"],
        metrics=metrics,
        protected_attribute="sex",
        n_bootstrap=100,
        rng=np.random.default_rng(42),
    ...
    t2d_boots = bootstrap_metrics(
        eval_df[eval_df["model"] == "T2D"],
        metrics=metrics,
        protected_attribute="sex",
        n_bootstrap=100,
        rng=np.random.default_rng(42),
    ...
    sczbp_boots = bootstrap_metrics(
        eval_df[eval_df["model"] == "SCZ/BP"],
        metrics=metrics,
        protected_attribute="sex",
        n_bootstrap=100,
        rng=np.random.default_rng(42),
    ...
    pr_boots = bootstrap_metrics(
        eval_df[eval_df["model"] == "PR"],
        metrics=metrics,
        protected_attribute="sex",
        n_bootstrap=100,
        rng=np.random.default_rng(42),
    ...
    ivc_boots = bootstrap_metrics(
        eval_df[eval_df["model"] == "IVC"],
        metrics=metrics,
        protected_attribute="sex",
        n_bootstrap=100,
        rng=np.random.default_rng(42),
