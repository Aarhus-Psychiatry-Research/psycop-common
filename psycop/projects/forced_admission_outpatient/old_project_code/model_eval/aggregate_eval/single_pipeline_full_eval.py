...
from psycop.projects.forced_admission_outpatient.old_project_code.model_eval.model_description.feature_importance.gain.gain_table import (
    generate_feature_importance_table,
)
from psycop.projects.forced_admission_outpatient.old_project_code.model_eval.model_description.performance.main_performance_figure import (
    fa_outpatient_create_main_performance_figure,
)
...
from psycop.projects.forced_admission_outpatient.old_project_code.model_eval.model_description.robustness.main_robustness_figure import (
    fa_outpatient_create_main_robustness_figure,
)
from psycop.projects.forced_admission_outpatient.old_project_code.model_eval.model_permuation.train_test_different_lookaheads import (
...
def fa_outpatient_create_markdown_artifacts(
    pipeline: ForcedAdmissionOutpatientPipelineRun,
...
def fa_outpatient_main_manuscript_eval(
    pipeline: ForcedAdmissionOutpatientPipelineRun,
) -> list[MarkdownArtifact]:
    msg.divider(
    ...
    fa_outpatient_output_performance_by_ppr(run=pipeline)
    fa_outpatient_create_main_performance_figure(run=pipeline)
    fa_outpatient_create_main_robustness_figure(run=pipeline)
    ...
    return fa_outpatient_create_markdown_artifacts(pipeline=pipeline)
...
if __name__ == "__main__":
    ...
    fa_outpatient_main_manuscript_eval(pipeline=get_best_eval_pipeline())  # type: ignore
