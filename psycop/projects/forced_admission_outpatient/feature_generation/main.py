...
from psycop.projects.forced_admission_outpatient.cohort.forced_admissions_outpatient_cohort_definition import (
    ForcedAdmissionsOutpatientCohortDefiner,
)
from psycop.projects.forced_admission_outpatient.feature_generation.modules.specify_features import (
    FeatureSpecifier,
)
from psycop.projects.forced_admission_outpatient.feature_generation.modules.specify_text_features import (
...
def main(
    ...
    if generate_in_chunks:
        flattened_df = ChunkedFeatureGenerator.create_flattened_dataset_with_chunking(
            project_info=project_info,
            eligible_prediction_times=ForcedAdmissionsOutpatientCohortDefiner.get_filtered_prediction_times_bundle(
        ...
        flattened_df = create_flattened_dataset_tsflattener_v1(
            feature_specs=feature_specs,  # type: ignore
            prediction_times_df=ForcedAdmissionsOutpatientCohortDefiner.get_filtered_prediction_times_bundle(
                washout_on_prior_forced_admissions=washout_on_prior_forced_admissions
            ).prediction_times.frame.to_pandas(),
...
if __name__ == "__main__":
    ...
    project_info = ProjectInfo(
        project_name="forced_admissions_outpatient",
        project_path=OVARTACI_SHARED_DIR / "forced_admissions_outpatient",
    )
