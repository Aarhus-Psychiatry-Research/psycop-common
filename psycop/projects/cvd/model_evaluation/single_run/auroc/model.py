...
class AUROC:
    ...
    tprs_lwoer: np.ndarray  # type: ignore
...
@dataclass(frozen=True)
class AUROCModel(SingleRunModel):
    n_bootstraps: int = 100

    def __call__(self, run: RunSelector) -> AUROC:
        eval_df = self._get_eval_df(run)
        ...
        tprs_bootstrapped, aucs_bootstrapped, base_fpr = bootstrap_roc(
            n_bootstraps=self.n_bootstraps, y=y, y_hat_probs=y_hat_probs
        )

        mean_tprs = tprs_bootstrapped.mean(axis=0)
        se_tprs = tprs_bootstrapped.std(axis=0) / np.sqrt(self.n_bootstraps)
        ...
        auc_se = np.std(aucs_bootstrapped) / np.sqrt(self.n_bootstraps)
