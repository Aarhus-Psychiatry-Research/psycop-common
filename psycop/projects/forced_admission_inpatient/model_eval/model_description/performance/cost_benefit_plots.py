...
def plot_cost_benefit_by_ppr(df: pd.DataFrame,
    ...
    p = (
        ...
        + pn.annotate('rect', xmin=float('-inf'), xmax=float('inf'), ymin=float('-inf'), ymax=0, fill='red', alpha=0.1) 
        + pn.annotate('rect', xmin=float('-inf'), xmax=float('inf'), ymin=0, ymax=float('inf'), fill='green', alpha=0.1)
    )
...
def cost_benefit_by_savings_recources_ratio(
    ...
    for savings in savings_from_prevented_outcome:
        ...
        savings_recources_ratio = f"{str(int(savings*efficiency_of_intervention))}:{str(cost_of_intervention)}"
