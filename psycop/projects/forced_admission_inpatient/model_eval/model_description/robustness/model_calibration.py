...
def calibration_plot(run: ForcedAdmissionInpatientPipelineRun):
    ...
    ax1.plot(prob_pred, prob_true, marker="o", color="#0072B2", label="Calibration Curve")  # Calibration line color
    ax1.plot([0, 1], [0, 1], linestyle="--", color="#D55E00", label="Perfectly Calibrated")  # Dotted line color
    ...
    ax2.bar(bin_edges[:-1], filtered_counts, width=np.diff(bin_edges), color="#0072B2", edgecolor="black")
    ax2.text(0.6, 0.95, 'Bins with <5 entries have been removed', color='red', fontsize=8,
    ...
    plt.tight_layout()
    calibration_curve_hist_path = run.paper_outputs.paths.figures / "fa_inpatient_calibration_curve_hist.png"
