...
from psycop.projects.forced_admission_inpatient.model_evaluation.figures.feature_importance.gain_plot import (
    plot_gain,
)


def gain_pipeline(model: Literal["baseline"], top_n: int = 20):
    ...
    if model == "baseline":
        ...
        raise ValueError(f"model is {model}, but must be 'baseline'")
