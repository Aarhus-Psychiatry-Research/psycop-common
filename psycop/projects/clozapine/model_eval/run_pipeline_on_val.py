...
from psycop.projects.clozapine.model_eval.config import BEST_POS_RATE, MODEL_NAME
from psycop.projects.clozapine.utils.pipeline_objects import ClozapinePipelineRun, RunGroup

msg = Printer(timestamp=True)  # type: ignore


def _get_test_run_name(pipeline_to_train: ClozapinePipelineRun) -> str:
    return f"{pipeline_to_train.name}-eval-on-test"  # add w wash if eval on cohort with washout


def _get_test_group_name(pipeline_to_train: ClozapinePipelineRun) -> str:
    return f"{pipeline_to_train.group.group_name!s}-eval-on-test"  # add w wash if eval on cohort with washout


def _get_test_group_path(pipeline_to_train: ClozapinePipelineRun) -> Path:
    return Path(
...
def _get_test_pipeline_dir(pipeline_to_train: ClozapinePipelineRun) -> Path:
...
def _train_pipeline_on_test(
    pipeline_to_train: ClozapinePipelineRun,
...
def test_selected_model_pipeline(
    pipeline_to_test: ClozapinePipelineRun,
    splits_for_training: list | None = None,  # type: ignore
    splits_for_evaluation: list | None = None,  # type: ignore
) -> ClozapinePipelineRun:
