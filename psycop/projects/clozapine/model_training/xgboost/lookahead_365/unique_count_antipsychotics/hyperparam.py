...
FEATURE_SETS = {"unique_antipsychotics_365d_lookahead": ["unique_count_antipsychotics"]}
...
def hyperparameter_search(cfg: PsycopConfig):
    ...
    for feature_set, features in FEATURE_SETS.items():
        cfg.mut(
            ...
            f"clozapine hparam, {feature_set}, xgboost, 1 year lookbehind filter, 2025_random_split, no_plasma_clozapine",
        )
...
if __name__ == "__main__":
    ...
    hyperparameter_search(
        PsycopConfig().from_disk(Path(__file__).parent / "clozapine_xgb_365d_unique_antipsych.cfg")
    )
