...
FEATURE_SETS = {
    "structured_text_365d_lookahead": [
        "contacts",
        "selvmord-broset",
        "diagnoses",
        "medication",
...
def hyperparameter_search(cfg: PsycopConfig):
    ...
    for feature_set, features in FEATURE_SETS.items():
        cfg.mut(
            ...
            f"clozapine hparam, no_clozapine_leponex_tfidf {feature_set}, xgboost, 1 year lookbehind filter, 2025_random_split, no_plasma_clozapine",
        )
...
if __name__ == "__main__":
    ...
    hyperparameter_search(
        PsycopConfig().from_disk(
            Path(__file__).parent / "no_clozapine_leponex_tfidf_xgb_365d_full_predictor.cfg"
        )
    )
