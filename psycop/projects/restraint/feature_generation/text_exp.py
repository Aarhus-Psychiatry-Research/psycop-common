...
def main():
    ...
    chunk2 = restraint_pred_times[int(len(restraint_pred_times)/3):int(len(restraint_pred_times)/3)*2]
    ...
    for count, chunk in enumerate([chunk1, chunk2, chunk3]):
        for note_type in ["aktuelt_psykisk", "all_relevant"]:
                for model_name in ["tfidf-500", "tfidf-1000", "dfm-encoder-large", "dfm-encoder-large-v1-finetuned"]:
                    feature_specs = TextFeatureSpecifier(project_info=project_info, min_set_for_debug=False).get_text_feature_specs(note_type = note_type, model_name = model_name)
                    ...
                    flattened_df.write_parquet(project_info.project_path / f"{note_type}_{model_name}_{count}.parquet")
