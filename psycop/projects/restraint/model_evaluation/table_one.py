...
class RestraintTableOne:
    ...
    def make_section_A(self, pred_times: pl.DataFrame) -> pd.DataFrame:
        ...
        pd_pred_times = (
            RestraintTableOne.add_split(pred_times)
            .select(
                [
                    r.source_col_name
    ...
    def make_section_B(self, pred_times: pl.DataFrame) -> pd.DataFrame:
        ...
        fns = [
            RestraintTableOne.add_sex,
            RestraintTableOne.add_bipolar,
            RestraintTableOne.add_scz,
            RestraintTableOne.add_time_of_first_contact,
            RestraintTableOne.add_time_from_first_contact_to_bp_diagnosis,
            RestraintTableOne.add_time_from_first_contact_to_scz_diagnosis,
            partial(RestraintTableOne.add_n_prediction_times, pred_times=pred_times),
            RestraintTableOne.add_n_admissions,
        ]
        for fn in fns:
            df = fn(df)

        split_by_id = RestraintTableOne.get_split_by_id(pred_times=pred_times)
...
if __name__ == "__main__":
    ...
    tables = RestraintTableOne(cfg=cfg).make_tables()  # noqa: ERA001
