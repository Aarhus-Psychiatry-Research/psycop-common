...
class RestraintTableOne:
    ...
    def patients_table(self, pred_times: pl.DataFrame) -> pd.DataFrame:
        ...
        agg_pred_times = (
            split_pred_times[
                [
                    "dw_ek_borger",
                    "timestamp_admission",
                    "pred_sex_female_fallback_nan",
                    "pred_age_years_fallback_nan",
                    "outc_all_restraint_within_2_days_boolean_fallback_0_dichotomous",
                    "outc_manual_restraint_within_2_days_boolean_fallback_0_dichotomous",
                    "outc_chemical_restraint_within_2_days_boolean_fallback_0_dichotomous",
                    "outc_mechanical_restraint_within_2_days_boolean_fallback_0_dichotomous",
                    "split",
                ]
            ]
            .group_by(
                ["dw_ek_borger", "timestamp_admission", "split", "pred_sex_female_fallback_nan"]
            )
            .agg(
                pl.col(
                    [
                ...
                pl.col("pred_age_years_fallback_nan").mean(),
            )
        )

        agg_pred_times = agg_pred_times.with_columns(
            pl.col("outc_all_restraint_within_2_days_boolean_fallback_0_dichotomous").replace(2, 1)
        )
        agg_pred_times = agg_pred_times.with_columns(
            pl.col("outc_manual_restraint_within_2_days_boolean_fallback_0_dichotomous").replace(
                2, 1
            )
        )
        agg_pred_times = agg_pred_times.with_columns(
            pl.col("outc_chemical_restraint_within_2_days_boolean_fallback_0_dichotomous").replace(
                2, 1
            )
        )
        agg_pred_times = agg_pred_times.with_columns(
        ...
        filtered_pred_times = agg_pred_times.select(
            [
                r.source_col_name
                for r in row_specifications
                if r.source_col_name not in ["age_grouped"]
        ...
        patient_frame = (
            ...
            .group_by(["dw_ek_borger", "split", "pred_sex_female_fallback_nan"])
            .agg(
                pl.col(
                    [
                        "outc_all_restraint_within_2_days_boolean_fallback_0_dichotomous",
...
if __name__ == "__main__":
    ...
    pred_times = pl.read_parquet(
        "E:/shared_resources/coercion/text/structured.parquet"
    )  # RestraintTableOne.get_filtered_prediction_times(self)
