...
def auroc_by_day_of_week(run: Run, path: Path):
    ...
    binned_df["proportion_in_bin"] = binned_df["n_in_bin"] / sum(binned_df.n_in_bin) * 5.65
    ...
    (
        ...
        + pn.labs(
            x="Day of the week", y="AUROC", title=f"{MODEL_NAME[run.name]} Performance Across Week"
        )
        + pn.coord_cartesian(ylim=(0.75, 0.97))
        + PN_THEME
        + pn.theme(axis_text_x=pn.element_text(rotation=90))
    ).save(path / "auc_by_day_of_the_week.png")
...
def auroc_by_month_of_year(run: Run, path: Path):
    ...
    binned_df["proportion_in_bin"] = binned_df["n_in_bin"] / sum(binned_df.n_in_bin) * 8.2
    binned_df["n_in_bin"] = binned_df["n_in_bin"].astype(int)
    ...
    (
        ...
        + PN_THEME
        + pn.theme(axis_text_x=pn.element_text(rotation=90))
    ).save(path / "auc_by_month_of_the_year.png")
