...
from psycop.projects.restraint.model_evaluation.figures.feature_importance.shap.shap_plots import (
    plot_shap_summary,
)
...
def shap_summary_pipeline(model: Literal["baseline", "text"], top_n: int = 20):
    ...
    cfg: FullConfigSchema = load_fullconfig(wandb_group=run.group.name, wandb_run=run.name)  # type: ignore
    ...
    shap_values = generate_shap_values(
        features=dataset[feature_cols], outcome=dataset[outcome_cols], pipeline=pipe
    )
