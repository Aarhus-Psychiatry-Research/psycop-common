...
from psycop.projects.restraint.evaluation.figures.auroc_by_age import (
    auroc_by_age_model,
    plotnine_auroc_by_age,
)
from psycop.projects.restraint.evaluation.figures.auroc_by_month import (
    auroc_by_month_model,
    plotnine_auroc_by_month,
)
from psycop.projects.restraint.evaluation.figures.auroc_by_sex import (
    auroc_by_sex_model,
    plotnine_auroc_by_sex,
)
from psycop.projects.restraint.evaluation.figures.auroc_by_week import (
...
def run_auroc_figures(df: pl.DataFrame, save_dir: Path, experiment: str):
    plotnine_auroc_by_age(
        auroc_by_age_model(
            df=df, birthdays=pl.from_pandas(birthdays()), bins=[18, *range(20, 70, 10)]
        )
    ).save(save_dir / f"{experiment}_auroc_by_age.png")  # type: ignore

    plotnine_auroc_by_month(auroc_by_month_model(df=df)).save(
        save_dir / f"{experiment}_auroc_by_month.png"
    )  # type: ignore

    plotnine_auroc_by_sex(auroc_by_sex_model(df=df, sex_df=pl.from_pandas(sex_female()))).save(
        save_dir / f"{experiment}_auroc_by_sex.png"
    )  # type: ignore

    plotnine_auroc_by_weekday(auroc_by_weekday_model(df=df)).save(
        save_dir / f"{experiment}_auroc_by_weekday.png"
    )  # type: ignore
...
def run_all_figures(df: pl.DataFrame, experiment: str, best_pos_rate: float, eval_dir: str):
    ...
    ).savefig(save_dir / f"{experiment}_plot_grid.png")


if __name__ == "__main__":
    experiment = "restraint_mechanical"
    eval_dir = (
        f"E:/shared_resources/restraint/eval_runs/{experiment}_tuning_best_run_evaluated_on_test"
    )
