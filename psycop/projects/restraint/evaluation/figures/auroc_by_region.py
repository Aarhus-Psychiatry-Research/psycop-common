...
from psycop.common.model_training_v2.trainer.preprocessing.steps.geographical_split._geographical_split import add_shak_to_region_mapping, load_shak_to_location_mapping
...
def plotnine_auroc_by_region(df: pd.DataFrame, title: str = "AUROC by Region") -> pn.ggplot:
    ...
    df["region_en"] = pd.Categorical(df['region'], categories=["West", "Central", "East"], ordered=True)
    ...
    p = (
        ...
        + pn.geom_text(pn.aes(x="region_en", y="proportion_of_n", fill="region_en", label="percentage_of_n"), va="bottom", format_string="{:.1f}%")
        + pn.geom_text(pn.aes(x="region_en", y="auroc", label="auroc"), nudge_y=0.09, va="top", format_string="{:.2f}")
        + pn.ylim(0, 1.01)
        + pn.theme_minimal()
        + pn.theme(
...
def auroc_by_region_model(df: pl.DataFrame) -> pd.DataFrame:
    pred_times = pl.DataFrame(load_restraint_prediction_timestamps()).select(pl.col(["dw_ek_borger", "timestamp", "timestamp_discharge"]))

    df_with_admission_timestamp = parse_timestamp_from_uuid(parse_dw_ek_borger_from_uuid(df)).with_columns(pl.col("timestamp").dt.cast_time_unit("ns")).join(pred_times, on=["dw_ek_borger", "timestamp"], how="left")
    ...
    eval_df = (
        df_with_admission_timestamp.join(sorted_all_visits_df, left_on=["dw_ek_borger", "timestamp_discharge"], right_on=["dw_ek_borger", "timestamp"], how="left")
    ).to_pandas()
    ...
    df = df.with_columns(pl.when(pl.col("region") == "vest").then(pl.lit("West")).when(pl.col("region") == "midt").then(pl.lit("Central")).otherwise(pl.lit("East")).alias("region"))
