...
def coercion_duration(
    ...
    sql = f"SELECT dw_ek_borger, datotid_start_sei, datotid_slut_sei, varighed_timer_sei, typetekst_sei FROM [fct].[FOR_tvang_alt_hele_kohorten_tvangsindlæggelse_sep_2025] WHERE datotid_start_sei IS NOT NULL AND typetekst_sei NOT IN {coercion_discard}"
    ...
    if coercion_type and reason_for_coercion:
        sql += (
            f" AND typetekst_sei = '{coercion_type}' AND begrundtekst_sei = '{reason_for_coercion}'"
        )
