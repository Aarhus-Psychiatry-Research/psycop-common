...
ORIGINAL_DATASET = BASE_DIR / "forced_admissions_inpatient" / "flattened_datasets"/"structured_feature_set"/"structured_feature_set.parquet"    
TEMPORAL_VAL_DATASET = BASE_DIR /"forced_admissions_inpatient_temp_val" / "flattened_datasets"/"structured_feature_set_temp_val.parquet"     

TIMESTAMP_COL = "timestamp" 
PATIENT_COL = "dw_ek_borger"

OUTPUT_FILE = BASE_DIR /"forced_admissions_inpatient_temp_val" / "flattened_datasets"/ "merged_original_temp_val_feature_set.parquet"
# -----------------------------------------
...
def merge_datasets():
    ...
    print(f"Old dataset: {ORIGINAL_DATASET.name} -> {len(df_original):,} rows, {df_original[PATIENT_COL].nunique():,} unique patients")
    print(f"New dataset: {TEMPORAL_VAL_DATASET.name} -> {len(df_temp_val):,} rows, {df_temp_val[PATIENT_COL].nunique():,} unique patients")
    ...
    if before_dedup != after_dedup:
        print(f"Total rows after merge (after dropping {before_dedup - after_dedup:,} exact duplicate rows): {after_dedup:,}")
    print(f"Total unique patients in merged data: {merged[PATIENT_COL].nunique():,}")
    print(f"New patients added from newest dataset ({TEMPORAL_VAL_DATASET.name}): {len(added_patients):,}")
    ...
    print(f"Earliest timestamp in temporal validation dataset ({TEMPORAL_VAL_DATASET.name}): {earliest_in_temp_val}")
