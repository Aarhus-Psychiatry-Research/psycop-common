...
from psycop.projects.forced_admission_inpatient_temp_val.cohort.forced_admissions_inpatient_cohort_definition import (
    ForcedAdmissionsInpatientTempValCohortDefiner,
)
from psycop.projects.forced_admission_inpatient_temp_val.feature_generation.modules.specify_features import (
    FeatureSpecifier,
)
...
def main(
    ...
    if generate_in_chunks:
        flattened_df = ChunkedFeatureGenerator.create_flattened_dataset_with_chunking(
            project_info=project_info,
            eligible_prediction_times=ForcedAdmissionsInpatientTempValCohortDefiner.get_filtered_prediction_times_bundle(
        ...
        flattened_df = create_flattened_dataset_tsflattener_v1(
            feature_specs=feature_specs,  # type: ignore
            prediction_times_df=ForcedAdmissionsInpatientTempValCohortDefiner.get_filtered_prediction_times_bundle(
                washout_on_prior_forced_admissions=washout_on_prior_forced_admissions
            ).prediction_times.frame.to_pandas(),
...
if __name__ == "__main__":
    ...
    project_info = ProjectInfo(
        project_name="forced_admissions_inpatient_temp_val",
        project_path=OVARTACI_SHARED_DIR / "forced_admissions_inpatient_temp_val",
    )
    ...
    main(
        ...
        feature_set_name="Min_set_debug_fa_temp_val_2025",
