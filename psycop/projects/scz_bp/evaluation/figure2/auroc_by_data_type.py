...
def scz_bp_auroc_by_data_type(modality2experiment_mapping: dict[str, str], model_type: Literal["joint", "scz", "bp"]) -> pl.DataFrame:
    auc_roc_dfs = []
    aucs: dict[str, float] = {}
    for modality, experiment_name in modality2experiment_mapping.items():
        eval_df = scz_bp_get_eval_ds_from_best_run_in_experiment(experiment_name=experiment_name, model_type=model_type)
...
def plot_scz_bp_auroc_by_data_type(modality2experiment: dict[str, str], model_type: Literal["joint", "scz", "bp"]) -> pn.ggplot:
    df = scz_bp_auroc_by_data_type(modality2experiment_mapping=modality2experiment, model_type=model_type)
...
if __name__ == "__main__":
    ...
    df = scz_bp_auroc_by_data_type(modality2experiment_mapping=modality2experiment, model_type="joint")
