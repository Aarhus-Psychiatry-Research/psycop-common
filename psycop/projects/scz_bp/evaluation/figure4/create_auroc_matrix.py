...
def scz_bp_merge_eval_dfs(eval_ds: EvalDataset) -> pl.DataFrame:
    ...
    meta_df_filtered = meta_df.with_columns(pl.concat_str(
        [
            pl.col("dw_ek_borger"),
            pl.col("timestamp"),
        ],
    ...
    meta_df_filtered.select("pred_time_uuid", "meta_scz_diagnosis_within_0_to_1825_days_max_fallback_0", "meta_bp_diagnosis_within_0_to_1825_days_max_fallback_0", "outc_first_scz_or_bp_within_0_to_1825_days_max_fallback_0")

    eval_df = eval_ds.to_polars().with_columns(pl.concat_str(
        [
            pl.col("ids"),
            pl.col("pred_timestamps"),
        ],
        separator="-",
    ).alias("pred_time_uuid"))
    ...
    return joined_df.rename({"meta_scz_diagnosis_within_0_to_1825_days_max_fallback_0": "scz_diagnosis", "meta_bp_diagnosis_within_0_to_1825_days_max_fallback_0": "bp_diagnosis", "outc_first_scz_or_bp_within_0_to_1825_days_max_fallback_0": "first_diagnosis"})
...
def scz_bp_auroc_by_outcome(model2validation_mapping: list[tuple[str, str]]) -> pd.DataFrame:
    ...
    for model_name, validation_outcome in model2validation_mapping:
        eval_df = scz_bp_merge_eval_dfs(scz_bp_get_eval_ds_from_best_run_in_experiment(experiment_name=model_name))
        eval_df = eval_df.filter(pl.col(validation_outcome).is_not_null())

        auroc_df.loc[model_name, validation_outcome] = roc_auc_score(y_true=eval_df[validation_outcome], y_score=eval_df["y_hat_probs"])  # type: ignore
