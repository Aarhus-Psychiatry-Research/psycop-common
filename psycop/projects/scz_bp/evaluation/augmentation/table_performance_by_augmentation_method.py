...
if __name__ == "__main__":
    ...
    for method, experiment in experiment_dicts.items():
        ...
        df = (
            (metric_frame.frame.filter(pl.col(metric_frame.metric_col_name) == "sample_multiplier"))
            .join(only_oof, on="run_name")
            .select(pl.col("value").alias("sample_multiplier"), "pretty_value")
            .with_columns(
                pl.lit(method).alias("Method"), pl.col("sample_multiplier").cast(pl.Int32)
            )
            .sort("sample_multiplier")
            .pivot(values="pretty_value", index="Method", columns="sample_multiplier")
        )
        method_dfs.append(df)

    method_df = pl.concat(method_dfs, how="vertical")
    method_df.to_pandas().to_html(
        SCZ_BP_EVAL_OUTPUT_DIR / "performance_by_augmentation_method_table.html"
    )
