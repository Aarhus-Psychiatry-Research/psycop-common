...
from psycop.common.model_training_v2.trainer.preprocessing.steps.row_filter_split import RegionalFilter
...
if __name__ == "__main__":
    ...
    text_df = pl.from_pandas(load_text_sfis(
        text_sfi_names=note_types, include_sfi_name=True, n_rows=None
    ))
    text_df = text_df.filter(pl.col("timestamp") < datetime(2016, 11, 1), pl.col("value").str.contains_any(word_list))
    ...
    only_outcome = text_df.filter(pl.col("outc") == 1, pl.col("timestamp") < pl.col("outcome_timestamp"))
    
    with pl.Config() as cfg:
        cfg.set_tbl_cols(-1)
        print(text_df["overskrift"].value_counts().with_columns((pl.col("count") / pl.col("count").sum() * 100).round(0).alias("procent")).drop("count").sort("procent", descending=True))
