...
def fit_from_config(sample_multiplier: int, cfg: Config) -> float:
...
def fit_ddpm_model(cfg: Config, sample_multiplier: int) -> float:
    cfg_copy = cfg.copy()
    cfg_copy["trainer"]["synthetic_data"]["n_samples"] = base_positive_samples * sample_multiplier
...
if __name__ == "__main__":
    ...
    for cfg_path in cfgs:
        cfg_file = Path(__file__).parent / "config" / "augmentation" / cfg_path
        ...
        Parallel(n_jobs=10)(
            delayed(fit_fn)(sample_multiplier=sample_multiplier)
