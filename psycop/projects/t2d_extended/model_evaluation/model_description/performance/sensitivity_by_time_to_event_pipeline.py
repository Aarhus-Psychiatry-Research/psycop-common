...
def t2d_extended_plot_sensitivity_by_time_to_event(df: pd.DataFrame) -> pn.ggplot:
...
def sensitivity_by_time_to_event(eval_dataset: EvalDataset) -> pn.ggplot:
    ...
    p = t2d_extended_plot_sensitivity_by_time_to_event(plot_df)

    return p


def t2d_extended_sensitivity_by_time_to_event(run: T2DExtendedPipelineRun) -> pn.ggplot:
...
if __name__ == "__main__":
    ...
    t2d_extended_sensitivity_by_time_to_event(run=get_best_eval_pipeline())
