"""Evaluate model performance for each test year of the T2D-extended project.
...
def plot_auc_roc(eval_dataset: EvalDataset, title: str = "", n_bootstraps: int = 100) -> pn.ggplot:
    ...
    return (
        ...
        + T2D_PN_THEME
        + auroc_label
        + pn.theme(figure_size=(5, 5), dpi=300)
    )
...
def confusion_matrix_plot(cfg: PsycopConfig) -> pn.ggplot:
    ...
    df = pd.DataFrame(
        {"true": eval_dataset.y, "pred": eval_dataset.get_predictions_for_positive_rate(0.03)[0]}
    )
