...
def t2d_extended_parse_static_feature(full_string: str) -> str:
...
def t2d_extended_parse_temporal_feature(full_string: str) -> str:
...
def t2d_extended_feature_name_to_readable(full_string: str) -> str:
...
def t2d_extended_generate_feature_importance_table(
    ...
    top_100_features = feature_table.head(100).with_columns(
        pl.col("Feature Name").apply(lambda x: t2d_extended_feature_name_to_readable(x))
    )
