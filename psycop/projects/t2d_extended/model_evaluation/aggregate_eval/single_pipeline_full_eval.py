...
from psycop.projects.t2d_extended.model_evaluation.model_description.performance.main_performance_figure import (
    t2d_extended_create_main_performance_figure,
)
from psycop.projects.t2d_extended.model_evaluation.model_description.performance.performance_by_ppr import (
    t2d_extended_output_performance_by_ppr,
)
from psycop.projects.t2d_extended.model_evaluation.model_description.robustness.main_robustness_figure import (
    t2d_extended_create_main_robustness_figure,
)
...
def t2d_extended_create_markdown_artifacts(pipeline: T2DExtendedPipelineRun) -> list[MarkdownArtifact]:
...
def t2d_extended_main_manuscript_eval(pipeline: T2DExtendedPipelineRun) -> list[MarkdownArtifact]:
    msg.divider(
    ...
    t2d_extended_output_performance_by_ppr(run=pipeline)
    t2d_extended_create_main_performance_figure(run=pipeline)
    t2d_extended_create_main_robustness_figure(run=pipeline)

    return t2d_extended_create_markdown_artifacts(pipeline=pipeline)
