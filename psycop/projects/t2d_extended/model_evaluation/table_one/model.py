...
@shared_cache().cache
def table_one_model_val(run: PsycopMlflowRun, sex_col_name: str) -> TableOneModel:
    cfg = run.get_config()
    ...
    pipeline: BaselinePreprocessingPipeline = filled["trainer"].validation_preprocessing_pipeline
    ...
    return TableOneModel(
        ...
        outcome_col_name=cfg["trainer"]["validation_outcome_col_name"],
        sex_col_name=sex_col_name,
    )


@shared_cache().cache
def table_one_model_train(run: PsycopMlflowRun, sex_col_name: str) -> TableOneModel:
    cfg = run.get_config()
