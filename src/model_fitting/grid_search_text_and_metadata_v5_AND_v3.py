    ...
    **kwargs,
    ...
    def __grid_search(df, c, sampling, **kwargs):
        ...
        if sampling is None:
            print("no sampling")
            pipe = Pipeline(
                steps=[("preprocessor", preprocessor), ("clf", clf(**kwargs))]
            )
        else:
            print("sampling")
        ...
        gs_clf = GridSearchCV(
            pipe, parameters, scoring=scoring, cv=cv, verbose=True, n_jobs=-1
        )  # run on all cores
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--data", help="what data to use", default="train.csv")
    parser.add_argument(
        "-c", "--clfs", help="What classifier should you use", nargs="+", default=["nb"]
    )
    parser.add_argument(
        "-rs",
        "--resampling",
    ...
    parser.add_argument(
        "-cv", "--cv", help="number of cross validation folds", default=5, type=int
    )

    parser.add_argument("-tc", "--text_column", help="columns for text", default="text")
    parser.add_argument(
        "-lc", "--label_column", help="columns for labels", default="labels"
    )
    parser.add_argument(
        "-sc", "--scoring", help="scoring function for grid search", default="roc_auc"
    )
    parser.add_argument(
        "-ss",
        "--sampling_strategy",
